...
def main():
    ...
    print("=" * 64)
    print("ONTOLOGY COHERENCE REPORT — kingdom_hearts2.owx")
    print("=" * 64)
    ...
    for p in sorted(onto["objprops"]):
        ...
        if not d or not r:
            # non bloccante: la pipeline salta le proprieta' senza dominio/codominio
            soft.append(f"object property {p} senza dominio/codominio (domain={d}, range={r}) -> ignorata dalla pipeline")
            print(f"    --  {p}: dominio/codominio mancante (ignorata)")
        else:
            print(f"    OK  {p}: {d} -> {r}")
    print()
    ...
    for p in sorted(onto["dataprops"]):
        ...
        if not d:
            soft.append(f"data property {p} senza dominio -> ignorata dalla pipeline")
            print(f"    --  {p}: dominio mancante (ignorata)")
        else:
    ...
    import re, html
    t = (BASE / "ontology" / "kingdom_hearts2.owx").read_text(encoding="utf-8")

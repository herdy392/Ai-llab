...
def rule_baseline():
    per_split = {sp: _rule_eval(_load_jsonl(GEN / f"kh2_verifier_{sp}.jsonl"))
                 for sp in ["train", "val", "test"] if (GEN / f"kh2_verifier_{sp}.jsonl").exists()}
    te = per_split.get("test", _rule_eval(_load_jsonl(GEN / "kh2_verifier_test.jsonl")))
...
def llm_validator(sample=60):
    if not LLM.available():
        return None
    rows = _load_jsonl(GEN / "kh2_verifier_test.jsonl")[:sample]
    tp = fp = fn = tn = 0
...
def gliner_eval(hf_id="urchade/gliner_medium-v2.1", threshold=0.5):
    ...
    p = GEN / "kh2_ner_test.jsonl"
    if not p.exists():
        return None, "GLiNER saltato: manca generated/kh2_ner_test.jsonl."
    try:
...
def glirel_eval(hf_id="jackboyla/glirel-large-v0", threshold=0.5, top_k=3):
    ...
    corpus = BASE / "data-input" / "kh2_corpus.txt"
    gold_p = GEN / "kh2_instance_triples.jsonl"
        ...
        for i, m in enumerate(models, 1):
            ...
            try:
                met = mod03.train_verifier(
                    ...
                    out_dir=BASE / "models" / f"kh2_verifier_{m['slug']}",
                ...
                try:
                    ...
                    if not args.retrain and extracted_path.exists():
                        ...
                        sents = [(f"kh2_corpus_s{i:03d}", s) for i, s in enumerate(
                            L.sentence_split((BASE / "data-input" / "kh2_corpus.txt").read_text(encoding="utf-8")), 1)]
                        extract_with_verifier(m["slug"], BASE / "models" / f"kh2_verifier_{m['slug']}" / "best_model", sents)
                except Exception as e:

# Armored Core 6 — NLP Ontology Pipeline

Pipeline NLP end-to-end costruita sull'ontologia di **Armored Core 6**: a partire da testo
...
## Struttura
```
ArmoredCore_ontology_2026/
├─ ontology/armoredcore.owx              copia "master" dell'ontologia
├─ requirements.txt
...
└─ nlp-detection/
   ├─ data-input/ac6_corpus.txt          testo ABox di partenza
   ├─ ontology/armoredcore.owx           ontologia usata dalla pipeline
   ├─ script/                            01..10 + ac6_lib.py + _schema_templates.json
...
```bat
cd nlp-detection
python script\10_query_model.py verifier --rel pilots --subject Character --object ArmoredCore

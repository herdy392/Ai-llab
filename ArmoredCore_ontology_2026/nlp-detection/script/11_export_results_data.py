...
REPO = Path(__file__).resolve().parents[2]          # -> ArmoredCore_ontology_2026/
GEN = BASE / "generated"
ONTO_FILE = BASE / "ontology" / "armoredcore.owx"
...
def _cls_compatible(sub_short: str, required_class: str) -> bool:
    """required_class deve essere antenato-o-uguale della classe sub_short."""
    return required_class == sub_short or required_class in onto["ancestors"](sub_short)
...
# ============================================================================
INVERSES = [("hasAlias", "isAliasOf"), ("hasRank", "isRankOf"),
            ("participatesIn", "involvesEntity"), ("isBossOf", "hasBoss"),
            ("takesPlaceIn", "hostsEvent"), ("pilots", "isPilotedBy"),
...
def export_ontology(out_dir: Path):
    ...
    report = {
        "ontology_file": "nlp-detection/ontology/armoredcore.owx",
        "counts": {
    ...
    lines = ["ONTOLOGY COHERENCE REPORT - armoredcore.owx", "=" * 56,
...
def export_dataset(out_dir: Path):
    ...
    for name in ["ac6_verifier_train.jsonl", "ac6_verifier_val.jsonl", "ac6_verifier_test.jsonl",
                 "ac6_verifier_all.jsonl", "ac6_verifier_stats.json",
                 "ac6_relation_candidates_train.jsonl", "ac6_relation_candidates_val.jsonl",
                 "ac6_relation_candidates_test.jsonl", "ac6_relation_candidates_all.jsonl",
                 "ac6_ner_train.jsonl", "ac6_ner_val.jsonl", "ac6_ner_test.jsonl",
                 "ac6_weak_stats.json", "ac6_instance_triples.jsonl"]:
    ...
    vstats = json.loads((GEN / "ac6_verifier_stats.json").read_text(encoding="utf-8"))
...
def export_metrics(out_dir: Path):
    ...
    test_rows = _load_jsonl(GEN / "ac6_verifier_test.jsonl")
    cm, appl, skip = _rule_verifier_confusion(test_rows)
    vm = _binary_metrics(cm)
    vm["evaluated_on"] = "ac6_verifier_test.jsonl"
    ...
    model_metrics = None
    model_dir = BASE / "models" / "ac6_verifier" / "best_model"
    try:
        import paths_config as _cfg  # type: ignore
    ...
    if model_dir.exists():
        try:
            ...
            model_metrics["evaluated_on"] = "ac6_verifier_test.jsonl"
...
"""
    ...
    md = f"""# Benchmarking Report — Armored Core 6 NLP Pipeline
...
## Contenuto cartelle
- `01_ontology/`: `armoredcore.owx`, `ontology_coherence_report.{{txt,json}}`, `ontology_interface.json`.
- `02_dataset/`: split verifier/relation/NER (`*_train/val/test.jsonl`), `synthetic_annotated_dataset/`,
  `ac6_instance_triples.jsonl` (gold), `dataset_statistics.json`, file `*_stats.json`.

...
Cosa fa:
  - rilegge i verifier JSONL (ac6_verifier_{train,val,test,all}) prodotti da 01;
...
# Sinonimi/parafrasi indirette per la tier "implicit" (per short relation).
PARAPHRASE = {
    "worksFor": ["is employed by", "serves under", "answers to"],
    "employs": ["has on its payroll", "gives orders to"],
    "hasAlias": ["is also known as", "goes by the name of"],
    "isAliasOf": ["is the codename of"],
    "hasRank": ["holds the rank of", "carries the rank of"],
    "isRankOf": ["is the rank held by"],
    "pilots": ["is at the controls of", "operates"],
    "isPilotedBy": ["is operated by"],
    "kills": ["eliminates", "takes down"],
    "isKilledBy": ["is taken down by"],
    "participatesIn": ["takes part in", "is involved in"],
    "involvesEntity": ["draws in", "features"],
    "isBossOf": ["leads the fight in", "is the boss encountered in"],
    "hasBoss": ["is led by"],
    "takesPlaceIn": ["unfolds in", "is set in"],
    "hostsEvent": ["is the setting of"],
    "isFriendlyTowards": ["is allied with", "stands with"],
    "isHostileToward": ["is at odds with", "opposes"],
}

DISTRACTORS = [
    ", according to the mission archive,",
    ", as several pilots later confirmed,",
    ", in a record stored on Rubicon,",
    ", a detail noted long after the operation,",
]

NESTED_PREFIX = [
    "Although the corporation had other plans,",
    "While a rival mercenary watched from afar,",
    "Even as the coral storm intensified,",
]
...
def main():
    ...
    raw = []
    for split in SPLITS:
        p = GEN / f"ac6_verifier_{split}.jsonl"
        if p.exists():
            raw += load(p)
    base = []
    for r in raw:
    ...
    for sp in SPLITS:
        ...
        dump(GEN / f"ac6_verifier_{sp}.jsonl", buckets[sp])
    ...
    dump(GEN / "ac6_verifier_all.jsonl", all_rows)
    ...
    # 5) Aggiorna ac6_verifier_stats.json con lo split REALE post-augmentation
    #    (l'export 11 legge questo file per verificare 70/15/15 e le label).
    from collections import Counter as _C
    vstats_p = GEN / "ac6_verifier_stats.json"
    if vstats_p.exists():

...
def train_verifier(model_name="bert-base-uncased", epochs=4, batch_size=16, lr=2e-5,
    ...
    out_dir = Path(out_dir) if out_dir else BASE / "models" / f"ac6_verifier_{slug}"
    ...
    train = load_jsonl(GEN / "ac6_verifier_train.jsonl")
    val = load_jsonl(GEN / "ac6_verifier_val.jsonl")
    test = load_jsonl(GEN / "ac6_verifier_test.jsonl")
...
def main():
    ...
    args = ap.parse_args()
    out = args.out or str(BASE / "models" / "ac6_verifier")

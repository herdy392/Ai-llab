...
BIO) on generated/ac6_ner_{train,val,test}.jsonl. Recognises AC6 entity types
(Character, Society, MajorEvent, Place, ArmoredCore, Alias, Rank).
...
def main():
    ...
    ap.add_argument("--out", default=str(BASE / "models" / "ac6_ner"))
    ...
    pin = dev["pin_memory"]
    train = load_jsonl(GEN / "ac6_ner_train.jsonl"); val = load_jsonl(GEN / "ac6_ner_val.jsonl"); test = load_jsonl(GEN / "ac6_ner_test.jsonl")

# MANUALE D'UTILIZZO — Armored Core 6 NLP Ontology Pipeline
...
| 03 | `03_train_verifier.py` | *(opzionale, GPU)* Allena il verifier BERT → `models/ac6_verifier/best_model`. |
| 04 | `04_train_ner.py` | *(opzionale, GPU)* Allena il NER → `models/ac6_ner/best_model`. |
...
**Per default riusa i training precedenti**: se in `models/ac6_verifier_<slug>/best_model`
...
| `VERIFIER_MODEL_DIR` | Cartella del verifier allenato | `r"D:\modelli\ac6_verifier\best_model"` |
| `NER_MODEL_DIR` | Cartella del NER allenato | `r"D:\modelli\ac6_ner\best_model"` |
